...
ドルマークで挟むと、文中に $\LaTeX$ の記法で数式 $e^{i \pi}$ を入れられます。
ドルマークを二つずつにすると、行が丸ごと数式に：
...
以下、関数読み込みがあるが、今回の目的はコードを読み解くことなので、直後のブロックの実行は切って `function.R` の中身をそのままコピペしておく：
```{r}
#| eval: false
# 自作関数の読み込み
